...
public class CommandController {
	...
	public ResponseEntity<Object> setValue3(@RequestBody CommandDto commandDto, @AuthenticationPrincipal Jwt jwt, Principal principal,
		...
		commandService.setValue(commandDto, UUID.fromString(requestId));
	...
	@ResponseBody
	public ResponseEntity<Object> addValue3(@RequestBody CommandDto commandDto, @AuthenticationPrincipal Jwt jwt, Principal principal,
											@RequestHeader("X-Request-Id") String requestId) {
		log.info("" + principal);
		log.info("" + commandDto);
		log.info(requestId);
		...
		commandDto.setEmail(getEmail());
		commandService.addValue(commandDto, UUID.fromString(requestId));
		return ResponseEntity.accepted().build();
	}
